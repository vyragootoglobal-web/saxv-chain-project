# saxv_chain_mini_v12.py
# SAXV Chain Mini V12 – Peer-to-Peer Network
...
class Blockchain:
    ...
    def mine_batch(self):
        ...
        batch = self.unconfirmed_transactions[:5]
...
wallet = Wallet()
peers = ["Node1", "Node2"]  # contoh peer list, bisa diperluas
...
def broadcast_transactions():
    while True:
        if blockchain.unconfirmed_transactions:
            # Simulate peer-to-peer broadcast
            for peer in peers:
                print(f"[Node {node_id}] Broadcasting {len(blockchain.unconfirmed_transactions)} tx to {peer}")
        time.sleep(5)
